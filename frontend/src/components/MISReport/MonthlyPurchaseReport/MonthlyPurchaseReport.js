...
import MonthlyPurchaseReportTable from "./MonthlyPurchaseReportTable";
import { useSelector, useDispatch } from "react-redux";
import swal from "sweetalert";
import moment from "moment";
import { getMonthlyPurchaseReport } from "../../../redux/actions/reportActions";
...
const MonthlyPurchaseReport = ({ show, onHide }) => {
  ...
  const submitHandler = (e) => {
    ...
    if (date) {
      dispatch(getMonthlyPurchaseReport(formData, headers));
      setTableMOdal(true);
    } else {
    ...
    >
      <Modal.Header className="background_and_table_header" closeButton>
        <Modal.Title id="example-modal-sizes-title-sm">
          <div>
            <h4 className=" responsive-head">Monthly Purchase {} Report</h4>
          </div>
        </Modal.Title>
      </Modal.Header>
