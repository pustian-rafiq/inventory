...
const MonthlySalesTable = React.forwardRef((props, ref) => {
  var headerData = props.headerTitle;
  const currentDate = new Date().toLocaleString();
  const { monthly_sales } = useSelector((state) => state.reports);

  // System information
  const sysInfo = monthly_sales
    .slice(monthly_sales.length - 1, monthly_sales.length)
    .map((data, i) => {
      return (
  ...
  const len1 = monthly_sales.length;
  const len2 = len1 == 2 ? len1 - 1 : len1 - 2;
  var summary = [];
  var totalSummary = [];
  // Show monthly sales total summary data
  if (len1 == 2) {
    summary = monthly_sales.slice(0, len2).map((data, i) => {
      return (
        <tr
          style={{
            fontSize: "16px",
    ...
    var monthlyData = monthly_sales.slice(0, len2).map((data, i) => {
      return (
        <tr key={i}>
          <td>{data.date}</td>
    ...
    totalSummary = monthly_sales.slice(len2, len1 - 1).map((data, i) => {
      return (
        <tr
          style={{
            fontSize: "16px",
    ...
    >
      <Modal.Header className="background_and_table_header" closeButton>
        <Modal.Title id="example-modal-sizes-title-sm">
          <div>
            <h4 className="responsive-head">Monthly Sales {headerData}</h4>
          </div>
        </Modal.Title>
      </Modal.Header>
