...
function StockComponent({ orderShow, StockList }) {
  ...
  const searchDatas = StockList.filter((search) => {
    return (
  ...
  var count = 0;
  const customerData = searchDatas.map((data, key) => {
    ++count;
    if (data) {
      return (
      ...
      <Modal.Body>
        <Nav variant="tabs" defaultActiveKey="/#">
          ...
          <Nav.Item>
            <Nav.Link
              eventKey="link-1"
              onClick={() => setPurchaseOrderShow(!purchaseOrderShow)}
            >
              Stock
            </Nav.Link>
          </Nav.Item>
            ...
            <div className="row">
              <div className="col-md-12 pt-3 ml-1 showBox1">
                ...
                <div className="printBtn pull-left">
                  ...
                  <span className="spanTitle" className="pull-right pt-2 pr-5 ">
                    Total: {count > 9 ? `${count} rows` : `${count} rows`}{" "}
                  </span>
                </div>
              </div>
