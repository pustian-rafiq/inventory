...
const PrintTable = React.forwardRef((props, ref) => {
  const { customer_return_report } = useSelector((state) => state.reports);
        ...
        <div className="report_modal_inner_content px-3">
          ...
          <div ref={ref} className="px-lg-5 mt-4">
            <div className="row text-center">
              <div className="col-lg-12">
                <h4 style={{ fontSize: "18px" }}>
                  Company Name:{" "}
                  {
                    customer_return_report[customer_return_report.length - 1]
                      ?.name
                  }
                </h4>
            ...
            <div className="text-center mt-3">
              <p>Customer Wise Return Report</p>
            </div>

            <table className="print-receipt">
              <thead>
                <tr className="thead">
                  <th style={style}>Return Date</th>
              ...
              {customer_return_report[0]?.map((returnData) => {
                return (
                  <tr>
                    <td>
                      {moment(returnData?.Return__created_date).format(
                        "YYYY-MM-DD"
                      )}
                    </td>
                    <td>{returnData?.product}</td>
                    <td>{returnData?.Quantity}</td>
                    <td>{returnData?.unit_price}</td>
                    <td>{returnData?.net_amounts}</td>
                  </tr>
                );
              })}
              ...
              >
                ...
                <td>{customer_return_report[1]?.Quantity}</td>
                <td></td>
                <td>{customer_return_report[1]?.net_amounts}</td>
              </tr>
            </table>
